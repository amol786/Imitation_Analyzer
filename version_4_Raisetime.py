...
location = r'E:\Project\bin\topology.csv'
topology = pd.read_csv(location)
topology = pd.DataFrame(topology)

resultFyle = open("E:\Project\output\output.csv",'w')
...
location = r'E:\Project\bin\systemNameall.csv'
...
location = r'E:\Project\alarm_files\alarm_dump\testing.csv'
df = pd.read_csv(location)
df = pd.DataFrame(df)
df.iloc[:,12] = df.iloc[:,12].fillna(value = df.iloc[:,1])
df.iloc[:,1] = pd.to_datetime(df.iloc[:,1],dayfirst=True,errors = 'coerce')
df.iloc[:,12] = pd.to_datetime(df.iloc[:,12],dayfirst=True,errors = 'coerce')
...
df = df.reindex(columns=['Severity', 'Raise time ( GMT )', 'System type','Clear time ( GMT )', 'System name', 'Supression Flag', 'Component','Additional text', 'Description', 'Acknowledged', 'Annotation',  'Alarm type', 'Probable cause', 'Equipment type',  'Cleared by', 'CLFI'])
...
while (start <= terminate):
	...
	for i, card in zip(list(range(0,pair)), l):
		...
		for index, row in window.iterrows():
			...
			#print(available)
			if available:
				mark[i].update({port:[1,row[0],row[1],row[2],row[4],row[5]]})
				#print(port,end = " : ")
				#print(mark[i][port])
				#print(row)
			#print(" ")
...
""" To calculate duration from the output1 file generated report 
so always run file on data analysis report file"""


location = r'E:\Project\output\output.csv'
col_names = ['Fiber_Name','Raise Time', 'Clear Time','Node_Name','Alarm','Card_No.','Fiber_No.']
report = pd.read_csv(location, names=col_names, header=None)
report = pd.DataFrame(report)

"""Adding Duration column or calculating Duration"""
 
report.iloc[:,1] = pd.to_datetime(report.iloc[:,1])
report.iloc[:,2] = pd.to_datetime(report.iloc[:,2])
report['Dration'] = report['Clear Time']-report['Raise Time']
